...
CREATE TABLE `customer_profiles` (`user_id` text PRIMARY KEY NOT NULL, `status` text DEFAULT 'active' NOT NULL, `country` text DEFAULT 'SA' NOT NULL, `phone` text, `last_seen_at` text NOT NULL, `created_at` text NOT NULL);
--> statement-breakpoint
...
--> statement-breakpoint
CREATE TABLE `invoices` (`id` text PRIMARY KEY NOT NULL, `user_id` text NOT NULL, `subscription_id` text, `reference` text NOT NULL UNIQUE, `subtotal_minor` integer NOT NULL, `discount_minor` integer DEFAULT 0 NOT NULL, `tax_minor` integer DEFAULT 0 NOT NULL, `total_minor` integer NOT NULL, `currency` text DEFAULT 'SAR' NOT NULL, `status` text DEFAULT 'pending' NOT NULL, `due_at` text NOT NULL, `paid_at` text, `created_at` text NOT NULL);
--> statement-breakpoint
CREATE TABLE `payments` (`id` text PRIMARY KEY NOT NULL, `user_id` text NOT NULL, `invoice_id` text, `reference` text NOT NULL UNIQUE, `amount_minor` integer NOT NULL, `currency` text DEFAULT 'SAR' NOT NULL, `method` text DEFAULT 'bank_transfer' NOT NULL, `status` text DEFAULT 'pending' NOT NULL, `settlement_status` text DEFAULT 'unsettled' NOT NULL, `occurred_at` text NOT NULL, `created_at` text NOT NULL);
--> statement-breakpoint
...
--> statement-breakpoint
CREATE TABLE `documents` (`id` text PRIMARY KEY NOT NULL, `owner_user_id` text NOT NULL, `space_id` text, `type` text NOT NULL, `reference` text NOT NULL UNIQUE, `person_name` text NOT NULL, `description` text NOT NULL, `amount_minor` integer DEFAULT 0 NOT NULL, `currency` text DEFAULT 'SAR' NOT NULL, `status` text DEFAULT 'issued' NOT NULL, `payment_method` text DEFAULT 'bank_transfer' NOT NULL, `approved_by` text, `issued_at` text NOT NULL, `created_at` text NOT NULL);
--> statement-breakpoint
