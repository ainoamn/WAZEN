...
## 8) استكمال 12 أغسطس (بعد الدمج) — قاعدة المنتج 20+فائض

| البند | الحالة |
|-------|--------|
| `splitContributionPayment` في `lib/finance.ts` | ✅ مع اختبارات (50→20+30) |
| API `recordContributionPayment` | ✅ يقسم الإلزامي/الفائض/التطوع/المقدم |
| API `withdrawSurplus` | ✅ سحب فائض دون خصم الصندوق المشترك |
| واجهة مساهمة بمعاينة التقسيم | ✅ في `TransactionModal` |
| `npm run setup:local` | ✅ توليد `.env.local` للتطوير |

*أُضيف في جلسة المتابعة بعد رفع v0.2.0.*
