...
| المشغّل | `BhdAppSwitcher` في `/home` · `/dashboard` · `AccountHeader` (فوترة/باقات/مستندات) · `AdminShell` — لا رابط `/admin` داخل المشغّل · الحساب → `https://id.bhd-om.com/account` — وإذا كان `GET /api/dashboard` بطيئًا/timeout، تعرض `/home` الهيدر/الأيقونات عبر `GET /api/auth` لتفادي شاشة “Sign in” بشكل خاطئ — والصفحة الرئيسية `/` تتحول إلى `AccountHeader` عند وجود جلسة وتعرض روابط تطبيقات BHD في فوتر داكن بنفس بنية فوتر BHD المركزي ومن الكتالوج نفسه |
