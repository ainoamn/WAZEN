...
- Schema patches (including TOTP pending columns) run when `schema_meta` is missing or behind `SCHEMA_VERSION` (currently 6). If the stored version is current, `ensureSchema` returns immediately — do not re-run patches on every request (that caused 30–50s add/void on Neon).
