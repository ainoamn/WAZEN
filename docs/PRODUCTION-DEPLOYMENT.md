...
ربط GitHub بمشروع Vercel ينشر كل دفع إلى `main` تلقائياً. التزام يبدأ بـ `docs:` يُتخطى (`ignoreCommand`) ولا يعيد البناء. العميل يقرأ `buildId` من `/api/health` كل 60 ثانية ويعيد التحميل عند تغيّر النشر، دون زر تحديث في المتصفح. بيانات اللوحة تُزامَن عبر `/api/dashboard?view=revision` كل 12 ثانية عندما تكون الصفحة ظاهرة (طلب خفيف بلا عدّ حصص).

بطاقة Vercel **Error** على التزام قديم (مثل `b94b94e`) ليست حالة الإنتاج إن كان `/api/health` يعرض `buildId` أحدث (`f3bea9c` في 9 سبتمبر 2026).
