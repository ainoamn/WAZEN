...
export function PublicHeader({ locale, setLocale }: { locale: CommerceLocale; setLocale: (locale: CommerceLocale) => void }) {
  ...
  return <header className="commerce-header">
    <Brand />
    <button className="commerce-menu" onClick={() => setOpen(!open)} aria-label="Menu">{open ? <X /> : <Menu />}</button>
    ...
    <div className="commerce-header-actions">
      ...
      <Link href="/dashboard">{l("تسجيل الدخول", "Sign in")}</Link>
      <Link className="filled" href="/dashboard">{l("ابدأ مجاناً", "Start free")}</Link>
    </div>
  </header>;
}
