...
function spaceLedger(space: Space, data: DashboardData) {
  const rows = data.transactions.filter((row) => row.space_id === space.id && isLiveTransaction(row) && !isPeerSettlementTransfer(row));
  const income = rows.filter((row) => ["income", "contribution"].includes(row.kind)).reduce((sum, row) => sum + row.amount_minor, 0);
  const spend = space.type === "trip"
    ? tripPostedSpendMinor(space.id, data.tripExpenses)
    : rows.filter((row) => row.kind === "expense").reduce((sum, row) => sum + row.amount_minor, 0);
  return { income, spend, remaining: space.type === "trip" ? space.balance_minor : income - spend };
}
...
function spaceMonthlyFlow(space: Space, data: DashboardData) {
  ...
  if (space.type === "trip") {
    const pendingOut = data.settlements
      .filter((row) => row.space_id === space.id && row.status === "pending" && !String(row.from_member_id).startsWith("space:") && !String(row.to_member_id).startsWith("space:"))
      .reduce((sum, row) => sum + (Number(row.amount_minor) || 0), 0);
    return { inflow: 0, outflow: 0, remainingIn: 0, remainingOut: pendingOut };
  }
  const plan = data.plans.find((item) => String(item.space_id) === space.id);
  const monthly = Number(plan?.amount_minor ?? 0);
  ...
  const recentSpend = data.transactions
    .filter((row) => row.space_id === space.id && row.kind === "expense" && isLiveTransaction(row) && !isPeerSettlementTransfer(row) && new Date(row.occurred_at).getTime() >= since)
...
function memberPosition(member: Member, data?: DashboardData, spaceId?: string) {
  ...
  if (data && expenseSpaceId) {
    ...
    for (const expense of data.tripExpenses.filter((item) => item.space_id === expenseSpaceId)) {
      ...
      const share = data.expenseSplits.find((row) => row.expense_id === expense.id && row.member_id === member.id);
    ...
    fundLeftoverMinor = pool.leftoverMinor;
    fundShortfallMinor = pool.shortfallMinor;
    settlementCreditMinor = Math.max(0, settleNet);
    settlementDebitMinor = Math.max(0, -settleNet);
    ...
    debit += settlementDebitMinor;
    credit += settlementCreditMinor;
  }
  const goalMinor = Number(member.due_minor) || 0;
  const paidTowardGoalMinor = goalMinor > 0 ? Math.min(paid, goalMinor) : paid;
  const paidAdvanceMinor = Math.max(0, paid - paidTowardGoalMinor);
  ...
  return {
    ...
    paidAdvanceMinor,
    ...net,
    debit: net.debitMinor,
    credit: net.creditMinor,
  };
}
...
/** Pending peer/fund settlements only (excludes fund-deficit rows covered by share netting). */
function memberSettlementNet(memberId: string, data: DashboardData, spaceId: string) {
  let net = 0;
  for (const settlement of data.settlements.filter((item) => item.space_id === spaceId && item.status === "pending")) {
    if (String(settlement.to_member_id).startsWith("space:")) continue;
    if (String(settlement.from_member_id).startsWith("space:")) {
      if (settlement.to_member_id === memberId) net += Number(settlement.amount_minor) || 0;
      continue;
    }
...
function dashboardError(code: string, locale: Locale) {
  const table = locale === "ar"
    ? {
      ...
      MEMBER_PHONE_TAKEN: "هذا الرقم مسجّل لمستخدم موجود. لا يُسمح بتكرار رقم الهاتف داخل الجمعية. للمتابعة حرّر بياناته.",
    ...
    : {
      ...
      MEMBER_PHONE_TAKEN: "This phone number already belongs to an existing member. Phone numbers cannot be repeated in this association. To continue, edit their details.",
...
export function WazenDashboard() {
  ...
  const totals = useMemo(() => {
    ...
    const liveRows = transactions.filter((item) => isLiveTransaction(item) && !isPeerSettlementTransfer(item));
    const spend = liveRows.filter((item) => item.kind === "expense").reduce((sum, item) => sum + item.amount_minor, 0)
...
function TransactionRow({ transaction, data, locale, onEdit, onVoid }: { transaction: Transaction; data: DashboardData; locale: Locale; onEdit?: (txn: Transaction) => void; onVoid?: (txn: Transaction) => void }) {
  ...
  return <div className={`transaction-row is-${transaction.kind}${isLiveTransaction(transaction) ? "" : " is-inactive"}`}>
...
function SpaceDetail({ space, data, locale, onAdd, onInvite, onEditWallet, onArchiveWallet, onTripExpense, onEditExpense, onCircleOrder, onClonePeriod, onClosePeriod, onReopenPeriod, onSettle, onCompleteTurn, onTxnChanged, onOpenMember }: { space: Space; data: DashboardData; locale: Locale; onAdd: () => void; onInvite: () => void; onEditWallet: () => void; onArchiveWallet: () => void; onTripExpense: () => void; onEditExpense: (expenseId: string) => void; onCircleOrder: () => void; onClonePeriod: () => void; onClosePeriod: () => void; onReopenPeriod: (periodId: string) => void; onSettle: (settlementId: string) => void; onCompleteTurn: (turnId: string) => void; onTxnChanged: (next: Partial<DashboardData>) => void; onOpenMember: (memberId: string, focus?: MemberLedgerFocus) => void }) {
  ...
  const tripUsesFundCash = space.type === "trip" && (
    members.some((item) => Number(item.paid_minor) > 0)
  ...
  const goalCollected = space.type === "personal" ? Math.max(0, space.balance_minor) : paidTotal;
  const progress = goal ? Math.max(0, Math.min(100, Math.round((goalCollected / goal) * 100))) : 0;
  ...
  const spentTotal = space.type === "trip"
    ? tripPostedSpendMinor(space.id, data.tripExpenses)
    : liveTransactions.filter((txn) => txn.kind === "expense").reduce((sum, txn) => sum + txn.amount_minor, 0);
  ...
  return <div className="dashboard-stack">
    ...
    <section className={`space-hero accent-${space.accent}`}>
      ...
      <div className="space-hero-facts">
        <div className="space-hero-facts-row">
          ...
          {space.type !== "personal" && <div className="hero-fact-violet"><i><Users size={16} /></i><div><small>{locale === "ar" ? "عدد الأعضاء" : "Members"}</small><b>{members.filter((member) => (member.status ?? "active") === "active").length}</b></div></div>}
          <div className="hero-fact-gold"><i><HandCoins size={16} /></i><div><small>{space.type === "personal" ? (locale === "ar" ? "إجمالي الدخل" : "Total income") : (locale === "ar" ? "إجمالي المدفوع" : "Total paid")}</small><b>{formatMoney(space.type === "personal" ? incomeTotal : paidTotal, space.currency, locale)}</b></div></div>
          ...
          {space.type === "personal" && <div className="hero-fact-lime"><i><Target size={16} /></i><div><small>{locale === "ar" ? "الهدف المالي" : "Goal"}</small><b>{goal ? formatMoney(goal, space.currency, locale) : "—"}</b></div></div>}
        </div>
        {space.type !== "personal" && (
        <div className="space-hero-facts-row">
    ...
    <section className="stat-grid compact">
      <StatCard icon={<TrendingUp />} label={locale === "ar" ? "إجمالي الدخل" : "Total income"} value={formatMoney(incomeTotal, space.currency, locale)} accent="green" note={space.type === "trip" ? (locale === "ar" ? "مساهمات الصندوق فقط — التحويلات بين الأعضاء ليست دخلاً" : "fund contributions only — peer transfers are not income") : (locale === "ar" ? "رواتب ومساهمات مرحلة" : "posted income and contributions")} />
      <StatCard icon={<TrendingDown />} label={locale === "ar" ? "إجمالي المصروف" : "Total spend"} value={formatMoney(spentTotal, space.currency, locale)} accent="rose" note={space.type === "trip" ? (locale === "ar" ? "فواتير الرحلة من الجيب أو الصندوق" : "trip bills from pocket or fund") : (locale === "ar" ? "مصروفات مرحلة فقط" : "posted expenses only")} />
      <StatCard icon={<CircleDollarSign />} label={locale === "ar" ? "المتبقي في هذه المحفظة" : "Remaining in this wallet"} value={formatMoney(remainingTotal, space.currency, locale)} accent="navy" note={space.type === "trip" ? (locale === "ar" ? "نقد الصندوق فقط — مصروف الجيب خارج الرصيد" : "fund cash only — pocket spend stays off the balance") : (locale === "ar" ? "دخل هذه المحفظة − صرفها" : "this wallet’s income − spend")} negative={remainingTotal < 0} positive={remainingTotal > 0} />
    ...
    {space.type !== "personal" && (() => {
      const flow = spaceMonthlyFlow(space, data);
      return <WalletForecastPanel locale={locale} currency={space.currency} spaceType={space.type} balanceMinor={space.balance_minor} monthlyInflowMinor={flow.inflow} monthlyOutflowMinor={flow.outflow} remainingInflowMinor={flow.remainingIn} remainingOutflowMinor={flow.remainingOut} foldId={`${space.id}:forecast`} />;
    })()}
    {space.type === "personal" && (
      <PersonalWalletPanel
        spaceId={space.id}
        ...
        activity={<SpaceTransactionsPanel space={space} data={data} locale={locale} onAdd={onAdd} onTxnChanged={onTxnChanged} />}
...
function MembersView({ data, locale, onInvite, onOpenPerson, onSmartPay, onMerged }: { data: DashboardData; locale: Locale; onInvite: () => void; onOpenPerson: (memberId: string, focus?: MemberLedgerFocus, editContact?: boolean) => void; onSmartPay: (memberId: string) => void; onMerged: (message: string) => void }) {
  ...
  return <div className="dashboard-stack"><div className="section-title"><div><h2>{t.memberProgress}</h2><p>{locale === "ar" ? "اضغط «تعديل» لتغيير الاسم أو الهاتف أو البريد. اسم العضو يفتح الملف المالي." : "Use Edit to change name, phone, or email. The member name opens the financial file."}</p></div><div className="section-title-actions"><DuplicateMergePanel members={groupMembers} spaces={societies} locale={locale} onMerged={onMerged} /><button className="secondary-button" onClick={() => onSmartPay(groupMembers[0]?.id ?? "")}><Sparkles size={16} />{locale === "ar" ? "المحاسب الذكي" : "Smart accountant"}{planHasFeature(planFeaturesOf(data), "smart_accountant") ? null : <PlanLockBadge locale={locale} />}</button><button className="primary-button" onClick={onInvite}><UserPlus size={17} />{t.invite}</button></div></div>
    <article className="panel members-panel person-table"><div className="panel-heading"><h2>{t.members} <span className="count-badge">{people.length}</span></h2><label className="search-field member-search"><Search size={16} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={locale === "ar" ? "ابحث باسم العضو" : "Search member name"} /></label></div><div className="members-table"><div className="table-head person-head"><span>{locale === "ar" ? "العضو" : "Member"}</span><span>{locale === "ar" ? "الحالة" : "Status"}</span><span>{locale === "ar" ? "الجمعيات" : "Associations"}</span><span>{locale === "ar" ? "الانضباط" : "Discipline"}</span><span>{locale === "ar" ? "عليه" : "Owes"}</span><span>{locale === "ar" ? "المستلم" : "Received"}</span><span>{locale === "ar" ? "إجراء" : "Action"}</span></div>{visible.map((records) => {
    ...
    const associationCount = new Set(records.map((row) => row.space_id)).size;
    const extraRows = records.length - associationCount;
    const currency = societies.find((space) => space.id === person.space_id)?.currency ?? "OMR";
    const associationLabel = locale === "ar"
      ? (associationCount === 1 ? "جمعية واحدة" : `${associationCount} جمعيات`)
      : String(associationCount);
    return <div className="member-row person-row" key={personIdentityKey(person)}><button type="button" className="member-name-hit" onClick={() => onOpenPerson(person.id, "all")}><div className="member-name"><i style={{ background: person.avatar }}>{person.display_name.slice(0, 1)}</i><div><strong>{person.display_name}</strong><span>{person.phone || person.email || (person.role === "owner" ? t.roleOwner : t.roleMember)}</span></div></div></button><span className={`status-pill ${active ? "complete" : "pending"}`}>{active ? <CheckCircle2 size={13} /> : <Clock3 size={13} />}{active ? (locale === "ar" ? "نشط" : "Active") : (locale === "ar" ? "غير نشط" : "Inactive")}</span><strong title={records.map((row) => societies.find((space) => space.id === row.space_id)?.name_ar || row.space_id).join(" · ")}>{associationLabel}{extraRows > 0 ? (locale === "ar" ? ` · مكرر ${extraRows}` : ` · +${extraRows}`) : ""}</strong><strong>{grade} · {rate}%</strong><button type="button" className={`amount-hit ${remaining ? "amount-negative" : "muted-amount"}`} onClick={() => onOpenPerson(person.id, "owes")}>{formatMoney(remaining, currency, locale)}</button><button type="button" className="amount-hit reserve-amount" onClick={() => onOpenPerson(person.id, "credit")}>{formatMoney(paid + extra, currency, locale)}</button><button type="button" className="secondary-button compact person-edit-hit" onClick={() => onOpenPerson(person.id, "all", true)}><Pencil size={14} />{locale === "ar" ? "تعديل" : "Edit"}</button></div>;
  })}</div></article>
...
function MembersTable({ members, locale, currency, data, spaceId, tripUsesFundCash, onWithdraw, onOpenMember, onChanged }: { members: Member[]; locale: Locale; currency: string; data?: DashboardData; spaceId?: string; tripUsesFundCash?: boolean; onWithdraw?: (memberId: string) => void; onOpenMember?: (memberId: string, focus?: MemberLedgerFocus) => void; onChanged?: (next: Partial<DashboardData>) => void }) {
  const t = copy[locale];
  const [query, setQuery] = useState("");
      ...
      <div className="members-table">
        <div className="table-head">
          ...
          <span title={data?.spaces.find((item) => item.id === spaceId)?.type === "trip"
            ? ((tripUsesFundCash || tripWalletUsesFundCash({
              spaceId: spaceId ?? "",
              expenses: data?.tripExpenses,
              membersPaidMinor: members.map((item) => item.paid_minor),
            }))
        ...
        {visible.map((member) => {
          ...
          const spaceType = data?.spaces.find((item) => item.id === spaceId)?.type;
          ...
          const usesFundCash = Boolean(tripUsesFundCash || (data && spaceId && tripWalletUsesFundCash({
            spaceId,
            expenses: data.tripExpenses,
            membersPaidMinor: members.map((item) => item.paid_minor),
          })));
          ...
          return (
            <div className={`member-row${!isActive ? " is-archived" : ""}`} key={member.id}>
              ...
              <button type="button" className="amount-hit" onClick={() => open("paid")}>
                <span className="claim-stack">
                  <strong>{formatMoney(paidMinor, currency, locale)}</strong>
                  {spaceType === "trip" && usesFundCash && pos.paidAdvanceMinor > 0 && (
                    <small>{locale === "ar"
                      ? `مساهمة ${formatMoney(pos.paidTowardGoalMinor, currency, locale)} · مقدّم ${formatMoney(pos.paidAdvanceMinor, currency, locale)}`
                      : `dues ${formatMoney(pos.paidTowardGoalMinor, currency, locale)} · advance ${formatMoney(pos.paidAdvanceMinor, currency, locale)}`}</small>
                  )}
                </span>
              </button>
              ...
              <span className={`status-pill ${!isActive ? "pending" : debit ? "pending" : "complete"}`}>
                ...
                {!isActive
                  ...
                  : debit ? (locale === "ar" ? "عليه مطالبات" : "Owes") : (credit ? (locale === "ar" ? "له رصيد" : "Credit") : (spaceType === "trip" ? (locale === "ar" ? "مسوّى" : "Settled") : t.paid))}
              </span>
...
function TransactionModal({ data, locale, preferredSpaceId, onClose, onSaved }: { data: DashboardData; locale: Locale; preferredSpaceId?: string; onClose: () => void; onSaved: (next: Partial<DashboardData>) => void }) {
  ...
  const [paidFrom, setPaidFrom] = useState<"common_fund" | "member" | "split">("common_fund");
  const [fundAmount, setFundAmount] = useState("");
  const [memberPocketAmount, setMemberPocketAmount] = useState("");
  const [splitMemberIds, setSplitMemberIds] = useState<string[]>(() =>
    data.members.filter((member) => member.space_id === initialSpace && (member.status ?? "active") === "active").map((member) => member.id)
  );
  const [expenseSplitMode, setExpenseSplitMode] = useState<"equal" | "mixed">("equal");
  const [sharedOnEveryone, setSharedOnEveryone] = useState("");
  const [remainderMemberIds, setRemainderMemberIds] = useState<string[]>([]);
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      if (kind === "expense" && space && space.type !== "personal" && expenseSplitMode === "mixed") {
        ...
        if (!preview) throw new Error(locale === "ar" ? "أدخل مبلغ الكل ثم علّم من يتحمل المتبقي." : "Enter the amount on everyone, then tick who covers the remainder.");
      }
      if (kind === "expense" && space && space.type !== "personal" && expenseSplitMode !== "mixed" && !splitMemberIds.length) {
        throw new Error(locale === "ar" ? "اختر من يتحمل هذا المصروف" : "Choose who this expense is for");
      }
      ...
      const mixedExpense = useGroupExpense && expenseSplitMode === "mixed" && space
        ? (() => {
          const totalMinor = parseMoneyToMinor(amount, space.currency);
          const sharedMinor = parseNonNegativeMoneyToMinor(sharedOnEveryone, space.currency);
          const extras = totalMinor - sharedMinor > 0 ? splitEvenly(totalMinor - sharedMinor, remainderMemberIds) : [];
          return {
            sharedAmount: sharedOnEveryone,
            sharedMemberIds: allIds,
            extraShares: extras.map((row) => ({ memberId: row.memberId, amount: currencyMajor(row.shareMinor, space.currency).toFixed(3) })),
          };
        })()
        : null;
      const response = await apiFetch("/api/dashboard", {
        ...
        body: JSON.stringify(useGroupExpense
          ? {
              ...
              spaceId,
              paidFrom,
              paidByMemberId: paidFrom === "member" || paidFrom === "split" ? memberId : undefined,
              ...(paidFrom === "split" ? { fundAmount, memberAmount: memberPocketAmount } : {}),
              ...(mixedExpense ?? { splitMemberIds: allIds }),
              amount,
...
function WalletModal({ data, locale, existing, defaultType = "trip", lockType = false, onClose, onSaved, onLiveData, onDeleted }: { data: DashboardData; locale: Locale; existing?: Space; defaultType?: string; lockType?: boolean; onClose: () => void; onSaved: (next: Partial<DashboardData>) => void; onLiveData?: (next: Partial<DashboardData>) => void; onDeleted?: (next: Partial<DashboardData>) => void }) {
  ...
  const [monthlyContribution, setMonthlyContribution] = useState(plan?.amount_minor ? String(Number(plan.amount_minor) / 1000) : "");
  const [durationMonths, setDurationMonths] = useState(plan?.duration_months ? String(plan.duration_months) : "");
  ...
  const contributionMinorPreview = Math.round(Number(monthlyContribution || 0) * 1000);
  const durationPreview = Math.max(0, Number(durationMonths) || 0);
  const liveGoalMinor = contributionMinorPreview > 0 && durationPreview > 0 ? contributionMinorPreview * durationPreview : 0;
  const features = planFeaturesOf(data);
    ...
    try {
      ...
      const response = await apiFetch("/api/dashboard", {
        ...
        body: JSON.stringify({
          ...
          ...(isGroup && String(monthlyContribution).trim()
...
function InviteModal({ data, locale, preferredSpaceId, onClose, onDone, onEditExisting }: { data: DashboardData; locale: Locale; preferredSpaceId?: string; onClose: () => void; onDone: (message: string) => void; onEditExisting: (memberId: string) => void }) {
  ...
  return <Modal title={locale === "ar" ? "إضافة مساهم" : "Add member"} onClose={onClose}>
    <form className="modal-form" onSubmit={submit}>
      ...
      <p className="modal-note">{locale === "ar" ? "رقم الهاتف لا يتكرر داخل الجمعية. البريد يمكن أن يتكرر. إن وُجد نفس الرقم تظهر ملاحظة لتحرير بياناته." : "Phone numbers cannot be repeated in this association. Emails may be repeated. If the number already exists, you will be asked to edit that member."}</p>
      <ContactSourceBar
        locale={locale}
        savedContacts={savedContacts}
        googleClientId={data.googleContactsClientId}
        onApply={(contact) => {
...
function ExpenseSplitPicker({
  ...
  const [mode, setMode] = useState<"everyone" | "pick">(forcePick || !isEveryone ? "pick" : "everyone");
  const [open, setOpen] = useState(forcePick || !isEveryone);
  const [query, setQuery] = useState("");
  ...
  try {
    const minor = amount.trim() ? parseMoneyToMinor(amount, currency) : 0;
    if (minor > 0 && orderedSelected.length) shares = splitEvenly(minor, orderedSelected);
  } catch { /* ignore */ }
  ...
  const note = (() => {
    ...
    if (orderedSelected.length === 1 && paidFrom === "member") {
      return locale === "ar"
        ? `${payerName} دفع عن ${beneficiary?.display_name}. الدافع له، والمستفيد عليه.`
        : `${payerName} paid for ${beneficiary?.display_name}. The payer is owed; the beneficiary owes.`;
    }
  ...
  const showList = forcePick || mode === "pick";
  return (
    <div className="expense-split-picker" ref={rootRef}>
      ...
      {!forcePick && <p className="modal-note">{note}</p>}
    </div>
  );
}
...
function trySharedRemainderPreview(input: {
  ...
  try {
    if (!input.sharedOnEveryone.trim()) return null;
    return composeSharedRemainder({
      totalMinor: parseMoneyToMinor(input.amount, input.currency),
      sharedMinor: parseNonNegativeMoneyToMinor(input.sharedOnEveryone, input.currency),
      remainderMemberIds: input.remainderMemberIds,
      allowedIds: input.members.map((member) => member.id),
    });
  } catch {
    return null;
  }
...
function GroupExpenseSplitFields({
  includeAmount,
  amount,
  onAmountChange,
  splitMode,
  ...
  splitMode: "equal" | "mixed";
  onSplitMode: (mode: "equal" | "mixed") => void;
  sharedOnEveryone: string;
  onSharedOnEveryone: (value: string) => void;
  remainderMemberIds: string[];
  onRemainderMemberIds: (ids: string[]) => void;
  members: Member[];
  locale: Locale;
  currency: string;
  paidFrom: "common_fund" | "member";
  ...
  let remainderLabel = "";
  let sharedTooHigh = false;
  try {
    if (splitMode === "mixed" && amount.trim() && sharedOnEveryone.trim()) {
      const totalMinor = parseMoneyToMinor(amount, currency);
      const sharedMinor = parseNonNegativeMoneyToMinor(sharedOnEveryone, currency);
      if (sharedMinor > totalMinor) sharedTooHigh = true;
      else remainderMinor = totalMinor - sharedMinor;
      remainderLabel = formatMoney(remainderMinor, currency, locale);
    }
  } catch { /* ignore */ }
  const preview = splitMode === "equal"
    ? (() => {
      try {
        const totalMinor = parseMoneyToMinor(amount, currency);
        const ids = members.map((member) => member.id);
        if (!ids.length || totalMinor <= 0) return null;
        return { totalMinor, splits: splitEvenly(totalMinor, ids) };
      } catch {
        return null;
      }
    })()
    : trySharedRemainderPreview({ amount, sharedOnEveryone, remainderMemberIds, members, currency });
  const showRemainderPicker = splitMode === "mixed" && Boolean(sharedOnEveryone.trim()) && remainderMinor > 0 && !sharedTooHigh;
  return (
    <>
      {includeAmount && <label><span>{locale === "ar" ? "المبلغ الكلي" : "Total amount"}</span><div className="money-input"><input required type="number" min="0.01" step="0.001" value={amount} onChange={(event) => onAmountChange(event.target.value)} /><b className="money-currency"><OmrSymbol size={14} /></b></div></label>}
      <div className="expense-split-mode">
        <button type="button" className={`secondary-button ${splitMode === "equal" ? "is-active" : ""}`} onClick={() => onSplitMode("equal")}>{locale === "ar" ? "الكل" : "Everyone"}</button>
        <button type="button" className={`secondary-button ${splitMode === "mixed" ? "is-active" : ""}`} onClick={() => onSplitMode("mixed")}>{locale === "ar" ? "اختيار من القائمة" : "Choose from list"}</button>
      </div>
      {splitMode === "equal" && <p className="modal-note">{locale === "ar" ? `يُقسَّم المبلغ الكلي مباشرة على كل الأعضاء (${members.length}).` : `The total is split equally among everyone (${members.length}).`}</p>}
      {splitMode === "mixed" && (
        <>
          <label><span>{locale === "ar" ? "المبلغ الذي سيُخصم على الكل" : "Amount charged to everyone"}</span><div className="money-input"><input type="number" min="0" step="0.001" value={sharedOnEveryone} onChange={(event) => onSharedOnEveryone(event.target.value)} placeholder="0.000" /><b className="money-currency"><OmrSymbol size={14} /></b></div></label>
          {sharedTooHigh && <p className="modal-error">{locale === "ar" ? "مبلغ الكل لا يمكن أن يتجاوز المبلغ الكلي." : "The shared amount cannot exceed the total."}</p>}
          {Boolean(sharedOnEveryone.trim()) && !sharedTooHigh && (
            <div className="split-remainder-banner">
              <span>{locale === "ar" ? "المتبقي" : "Remainder"}</span>
          ...
          {showRemainderPicker && (
            <ExpenseSplitPicker
              members={members}
              selectedIds={remainderMemberIds}
              onChange={onRemainderMemberIds}
              locale={locale}
              amount={currencyMajor(remainderMinor, currency).toFixed(3)}
              currency={currency}
              paidFrom={paidFrom}
              paidByMemberId={paidByMemberId}
              heading={locale === "ar" ? `من يتحمل المتبقي ${remainderLabel}؟` : `Who covers the remaining ${remainderLabel}?`}
              forcePick
            />
          )}
        </>
      ...
      {preview && (
        <p className="modal-note">{locale === "ar"
          ? `حصة كل عضو: ${preview.splits.map((row) => `${members.find((member) => member.id === row.memberId)?.display_name ?? row.memberId} ${formatMoney(row.shareMinor, currency, locale)}`).join(" · ")}`
          : `Each share: ${preview.splits.map((row) => `${members.find((member) => member.id === row.memberId)?.display_name ?? row.memberId} ${formatMoney(row.shareMinor, currency, locale)}`).join(" · ")}`}</p>
      )}
    </>
  );
}
...
function TripExpenseModal({ data, locale, preferredSpaceId, expenseId, onClose, onSaved }: { data: DashboardData; locale: Locale; preferredSpaceId?: string; expenseId?: string; onClose: () => void; onSaved: (next: Partial<DashboardData>) => void }) {
  ...
  const existingSplitRows = existing
    ...
    : [];
  const inferred = inferSharedRemainder(existingSplitRows, members.map((member) => member.id));
  const [paidFrom, setPaidFrom] = useState<"common_fund" | "member" | "split">(existing?.paid_from === "common_fund" ? "common_fund" : existing ? "member" : "common_fund");
  const [paidByMemberId, setPayer] = useState(existing?.paid_by_member_id ?? members[0]?.id ?? "");
  const existingAmountLabel = existing ? currencyMajor(existing.amount_minor, data.spaces.find((item) => item.id === existing.space_id)?.currency ?? "OMR").toFixed(3) : "";
  const [fundAmount, setFundAmount] = useState(existing?.paid_from === "common_fund" ? existingAmountLabel : "");
  const [memberAmount, setMemberAmount] = useState(existing && existing.paid_from !== "common_fund" ? existingAmountLabel : "");
  const [splitMode, setSplitMode] = useState<"equal" | "mixed">(inferred.mode);
  const [sharedOnEveryone, setSharedOnEveryone] = useState(inferred.mode === "mixed" ? currencyMajor(inferred.sharedMinor, currency).toFixed(3) : "");
  const [remainderMemberIds, setRemainderMemberIds] = useState<string[]>(inferred.remainderIds);
    ...
    try {
      ...
      if (splitMode === "mixed" && mixedPreview) {
        ...
        payload = {
          ...
          sharedAmount: sharedOnEveryone,
          sharedMemberIds: allIds,
          extraShares: extras.map((row) => ({ memberId: row.memberId, amount: currencyMajor(row.shareMinor, currency).toFixed(3) })),
        };
      } else {
        payload = { splitMemberIds: allIds, amount };
      }
      if (paidFrom === "split") {
        payload.fundAmount = fundAmount;
        payload.memberAmount = memberAmount;
      }
      const pocketPayer = paidFrom === "member" || paidFrom === "split" ? paidByMemberId : undefined;
      const response = await apiFetch("/api/dashboard", {
        method: "POST",
        headers: { "content-type": "application/json" },
    ...
    <Modal title={existing ? (locale === "ar" ? "تعديل مصروف جماعي" : "Edit group expense") : (locale === "ar" ? "إضافة مصروف جماعي" : "Add group expense")} wide={splitMode === "mixed"} onClose={onClose}>
      <form className="modal-form" onSubmit={submit}>
        ...
        {paidFrom === "split" && (
          <>
            <div className="form-row">
              <label><span>{locale === "ar" ? "مبلغ الصندوق" : "Fund amount"}</span><div className="money-input"><input required type="text" inputMode="decimal" autoComplete="off" value={fundAmount} onChange={(event) => {
                const next = applyTypedFundShare({ typed: event.target.value, totalMinor: billMinor, fundCashMinor, currency });
                if (!next) return;
                setFundAmount(next.fundAmount);
                if (next.memberAmount != null) setMemberAmount(next.memberAmount);
              }} onBlur={() => {
                const next = finalizeTypedFundShare({ typed: fundAmount, totalMinor: billMinor, fundCashMinor, currency });
                setFundAmount(next.fundAmount);
                if (next.memberAmount != null) setMemberAmount(next.memberAmount);
              }} /><b className="money-currency"><OmrSymbol size={14} /></b></div></label>
              <label><span>{locale === "ar" ? "مبلغ حساب العضو" : "Member amount"}</span><div className="money-input"><input required type="text" inputMode="decimal" autoComplete="off" value={memberAmount} onChange={(event) => setMemberAmount(event.target.value.replace(/[^\d.]/g, ""))} /><b className="money-currency"><OmrSymbol size={14} /></b></div></label>
            </div>
        ...
        <GroupExpenseSplitFields
          ...
          onRemainderMemberIds={setRemainderMemberIds}
          members={members}
          locale={locale}
          currency={currency}
          paidFrom={paidFrom === "common_fund" ? "common_fund" : "member"}
          paidByMemberId={paidByMemberId}
        />
