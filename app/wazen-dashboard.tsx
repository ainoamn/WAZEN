    ...
    <Modal title={existing ? (locale === "ar" ? "تعديل مصروف جماعي" : "Edit group expense") : (locale === "ar" ? "إضافة مصروف جماعي" : "Add group expense")} onClose={onClose}>
      <form className="modal-form" onSubmit={submit}>
        ...
        <p className="modal-note">{existing
          ...
          ? (locale === "ar" ? "يُخصم من صندوق الجمعية. الحصص للتوضيح فقط؛ «عليه» يظهر فقط إذا صار رصيد الصندوق سالباً (عجز). لدفع عضو من جيبه اختر «عضو» ليظهر له/عليه." : "Deducted from the association fund. Shares are informational; Owes appears only if the fund balance goes negative. Choose «Member» payer so Owes/Credit settlements are created.")
