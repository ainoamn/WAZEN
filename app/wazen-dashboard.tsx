    ...
    <Modal title={existing ? (locale === "ar" ? "تعديل مصروف جماعي" : "Edit group expense") : (locale === "ar" ? "إضافة مصروف جماعي" : "Add group expense")} onClose={onClose}>
      <form className="modal-form" onSubmit={submit}>
        ...
        <p className="modal-note">{existing
          ...
          ? (locale === "ar" ? "يُخصم من رصيد صندوق الجمعية فقط، وليس من حساب أي عضو. إن لم يكفِ النقد يُحمَّل العجز على الأعضاء للصندوق." : "Deducted from the association fund only, not from any member. If cash is short, the remainder is billed to members as dues to the fund.")
