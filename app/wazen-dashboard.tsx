...
function memberExpenseNet(memberId: string, data: DashboardData, spaceId: string) {
  ...
  for (const settlement of data.settlements.filter((item) => item.space_id === spaceId && item.status === "pending")) {
    // Fund-deficit rows are covered by fund expense shares below.
  ...
  for (const expense of data.tripExpenses.filter((item) => item.space_id === spaceId)) {
    ...
    if (shareMinor > 0) net -= shareMinor;
  }
  return net;
}
    ...
    <Modal title={existing ? (locale === "ar" ? "تعديل مصروف جماعي" : "Edit group expense") : (locale === "ar" ? "إضافة مصروف جماعي" : "Add group expense")} onClose={onClose}>
      <form className="modal-form" onSubmit={submit}>
        ...
        <p className="modal-note">{existing
          ...
          ? (locale === "ar" ? "يُخصم من صندوق الجمعية. حصة كل عضو تُحسب في «عليه». «له» يظهر عند دفع عضو من جيبه عن غيره أو عند وجود فائض." : "Deducted from the association fund. Each member share counts under Owes. Credit appears when a member paid from pocket for others or holds surplus.")
