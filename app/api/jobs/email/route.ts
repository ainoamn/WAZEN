...
function authorized(request: Request) {
  const secret = process.env.WAZEN_JOB_SECRET ?? ""; const supplied = request.headers.get("authorization") ?? "";
...
export async function POST(request: Request) {
  try {
    authorized(request);
    const configuredEndpoint = process.env.WAZEN_EMAIL_WEBHOOK_URL; const token = process.env.WAZEN_EMAIL_WEBHOOK_TOKEN;
    ...
    for (const message of pending.results) {
      try {
        ...
        await db.prepare("UPDATE email_outbox SET status='sent',attempts=attempts+1,sent_at=? WHERE id=? AND status='pending'").bind(new Date().toISOString(), message.id).run(); sent += 1;
      } catch {
