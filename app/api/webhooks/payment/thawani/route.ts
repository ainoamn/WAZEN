...
import { mapThawaniWebhook } from "../../../../../lib/payment-checkout";
...
export async function POST(request: Request) {
  try {
    const secret = process.env.WAZEN_THAWANI_WEBHOOK_SECRET?.trim()
    ...
    const supplied = (request.headers.get("x-wazen-signature")
      || request.headers.get("thawani-signature")
      || "").toLowerCase();
    if (supplied) {
    ...
    } else if (process.env.WAZEN_THAWANI_ALLOW_UNSIGNED !== "1") {
      // Prefer signed webhooks; unsigned only when explicitly allowed for UAT.
      throw new ApiError(401, "INVALID_SIGNATURE");
    }

    const parsed = mapThawaniWebhook(JSON.parse(raw));
