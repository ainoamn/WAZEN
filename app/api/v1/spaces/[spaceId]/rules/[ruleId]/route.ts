...
import { deleteV1PersonalRule, updateV1PersonalRule } from "../../../../../../../lib/v1-rules";
...
export async function PATCH(
  request: Request,
  context: { params: Promise<{ spaceId: string; ruleId: string }> },
) {
  return withRequestTiming("v1.rules.patch", async () => {
    const claimRef: { current: { db: D1Database; userId: string; key: string } | null } = { current: null };
    try {
      enforceWriteRequest(request);
      const { spaceId, ruleId } = await context.params;
      const db = getRawDb();
      ...
      return await runWithDbUser(user.id, async () => {
        ...
        const parsed = z.object({
          ...
          endsAt: z.string().min(8).max(40).optional().nullable(),
        ...
        const replay = await claimIdempotency(db, user.id, "v1.updateRule", idempotencyKey);
        if (replay) {
        ...
        const rule = await updateV1PersonalRule(db, user, space, ruleId, parsed.data);
        const response = { api: "wazen.v1", ok: true, rule };
        await enqueueIntegrationEvent(db, space.owner_user_id, "rule.updated", {
          spaceId: space.id,
          ruleId: rule.id,
          status: rule.status,
        }).catch(() => {});
...
export async function DELETE(
  ...
  return withRequestTiming("v1.rules.delete", async () => {
    const claimRef: { current: { db: D1Database; userId: string; key: string } | null } = { current: null };
    try {
      enforceWriteRequest(request);
      const { spaceId, ruleId } = await context.params;
      ...
      return await runWithDbUser(user.id, async () => {
        ...
        const replay = await claimIdempotency(db, user.id, "v1.deleteRule", idempotencyKey);
        if (replay) {
          const body = replay && typeof replay === "object" ? replay : { ok: true };
          return Response.json({ ok: true, ...body }, { headers: { "Cache-Control": "no-store", "X-Wazen-Api": "v1" } });
        }
        ...
        const result = await deleteV1PersonalRule(db, user, space.id, ruleId);
        const response = { api: "wazen.v1", ok: true, ...result };
        await enqueueIntegrationEvent(db, space.owner_user_id, "rule.deleted", {
          spaceId: space.id,
          ruleId: result.id,
        }).catch(() => {});
        await completeIdempotency(db, user.id, idempotencyKey, response);
