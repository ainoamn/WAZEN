...
import { createV1ReceiptShare } from "../../../../../../../lib/v1-shares";
...
export async function POST(
  ...
  return withRequestTiming("v1.shares.receipt", async () => {
    ...
    try {
      ...
      return await runWithDbUser(user.id, async () => {
        ...
        const replay = await claimIdempotency(db, user.id, "v1.shareReceipt", idempotencyKey);
        if (replay) {
        ...
        const share = await createV1ReceiptShare(db, user, space, {
          ...parsed.data,
