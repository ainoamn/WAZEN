...
import { createV1AssociationStatementShare } from "../../../../../../../lib/v1-shares";
...
export async function POST(
  ...
  return withRequestTiming("v1.shares.associationStatement", async () => {
    ...
    try {
      ...
      return await runWithDbUser(user.id, async () => {
        ...
        const replay = await claimIdempotency(db, user.id, "v1.shareAssociationStatement", idempotencyKey);
        if (replay) {
        ...
        const share = await createV1AssociationStatementShare(db, user, space, {
          ...parsed.data,
