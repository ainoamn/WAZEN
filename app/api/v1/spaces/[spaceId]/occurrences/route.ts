...
import { queueV1PersonalOccurrence } from "../../../../../../lib/v1-occurrences";
...
export async function POST(
  ...
  return withRequestTiming("v1.occurrences.queue", async () => {
    ...
    try {
      ...
      return await runWithDbUser(user.id, async () => {
        ...
        const replay = await claimIdempotency(db, user.id, "v1.queueOccurrence", idempotencyKey);
        if (replay) {
        ...
        const occurrence = await queueV1PersonalOccurrence(db, user, space, parsed.data);
        const response = { api: "wazen.v1", ok: true, occurrence };
        await enqueueIntegrationEvent(db, space.owner_user_id, "occurrence.queued", {
          spaceId: space.id,
