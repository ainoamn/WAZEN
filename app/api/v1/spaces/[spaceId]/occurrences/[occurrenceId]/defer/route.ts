...
import { deferV1PersonalOccurrence } from "../../../../../../../../lib/v1-occurrences";
...
export async function POST(
  ...
  return withRequestTiming("v1.occurrences.defer", async () => {
    ...
    try {
      ...
      return await runWithDbUser(user.id, async () => {
        ...
        const replay = await claimIdempotency(db, user.id, "v1.deferOccurrence", idempotencyKey);
        if (replay) {
        ...
        const occurrence = await deferV1PersonalOccurrence(
          db, user, space.id, occurrenceId, parsed.data.deferUntil,
        );
        const response = { api: "wazen.v1", ok: true, occurrence };
        await enqueueIntegrationEvent(db, space.owner_user_id, "occurrence.deferred", {
          spaceId: space.id,
