...
import { resplitV1Expenses } from "../../../../../../../lib/v1-expenses";
...
export async function POST(
  ...
  return withRequestTiming("v1.expenses.resplit", async () => {
    ...
    try {
      ...
      return await runWithDbUser(user.id, async () => {
        ...
        const replay = await claimIdempotency(db, user.id, "v1.resplitExpenses", idempotencyKey);
        if (replay) {
        ...
        const result = await resplitV1Expenses(db, user, space, parsed.data.expenseId);
        const response = { api: "wazen.v1", ok: true, ...result };
        await enqueueIntegrationEvent(db, space.owner_user_id, "expense.resplit", {
          spaceId,
