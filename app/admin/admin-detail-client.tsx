    ...
    <AdminConsole>
      ...
      <section className="admin-panel">
        ...
        {Array.isArray(profile.effective_features) && (
          <p style={{ marginTop: 12 }}><small>{l("الصلاحيات الفعلية", "Effective entitlements")}: {(Array.isArray(profile.effective_features) ? profile.effective_features.map((id) => PLAN_FEATURE_CATALOG.find((feature) => feature.id === id)?.[locale === "ar" ? "ar" : "en"] ?? String(id)).join(locale === "ar" ? "، " : ", ") : "—") || "—"} · {l("محافظ", "wallets")} {String(profile.effective_wallet_limit ?? "—")} · {l("أعضاء", "members")} {String(profile.effective_member_limit ?? "—")} · {l("مستخدمون", "users")} {String(profile.effective_user_limit ?? "—")} · {l("معاملات", "transactions")} {String(profile.effective_transaction_limit ?? "—")} · {l("يومياً", "daily")} {String(profile.effective_daily_transaction_limit ?? "—")} · {l("شهرياً", "monthly")} {String(profile.effective_monthly_transaction_limit ?? "—")} · {l("سجلات", "records")} {String(profile.effective_record_limit ?? "—")} · {l("مطبوعات", "prints")} {String(profile.effective_print_limit ?? "—")}</small></p>
        )}
