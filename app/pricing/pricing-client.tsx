...
export function PricingClient() {
  ...
  return <main className="pricing-page"><PublicHeader locale={locale} setLocale={setLocale} />
    ...
    <section className="pricing-grid">{plans.map((plan, index) => { const price = annual ? plan.annual_minor / 12 : plan.monthly_minor; return <article className={index === 2 ? "popular" : ""} key={plan.id}>{index === 2 && <span className="popular-label">{l("الأكثر اختياراً", "Most popular")}</span>}<small>{l("باقة", "Plan")}</small><h2>{locale === "ar" ? plan.name_ar : plan.name_en}</h2><p>{locale === "ar" ? plan.description_ar : plan.description_en}</p><div className="plan-price"><b>{plan.monthly_minor === 0 ? l("مجاناً", "Free") : <span className="omr-amount inline-flex items-center gap-1"><OmrSymbol className="h-5 w-auto" />{money(Math.round(price), locale)}</span>}</b>{plan.monthly_minor > 0 && <span>/ {l("شهرياً", "month")}</span>}</div>{annual && plan.monthly_minor > 0 && <em className="annual-note">{l("تدفع سنوياً", "Billed annually")} · {money(plan.annual_minor, locale)}</em>}<button disabled={working === plan.id} onClick={() => void selectPlan(plan.id)}>{working === plan.id ? l("جارٍ التجهيز...", "Preparing...") : plan.monthly_minor === 0 ? l("ابدأ مجاناً", "Start free") : l("اختر الباقة", "Choose plan")}</button><ul><li><Check/>{plan.wallet_limit >= 9999 ? l("محافظ غير محدودة", "Unlimited wallets") : `${plan.wallet_limit} ${l("محافظ", "wallets")}`}</li><li><Check/>{plan.member_limit >= 9999 ? l("أعضاء غير محدودين", "Unlimited members") : `${plan.member_limit} ${l("عضواً", "members")}`}</li>{plan.features.map((feature) => <li key={feature}><Check/>{featureCopy[feature]?.[locale === "ar" ? 0 : 1] ?? feature}</li>)}</ul></article>; })}</section>
