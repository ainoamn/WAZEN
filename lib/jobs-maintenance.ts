/** Shared maintenance job body used by /api/jobs/maintenance and /api/jobs/tick. */
...
export async function runMaintenanceJob(db: D1Database) {
  ...
  let rotated = 0;
  let auditRowsSanitized = 0;
  if (process.env.WAZEN_ENCRYPTION_KEYRING) {
    const keyring = loadKeyring();
    const statements: D1PreparedStatement[] = [];
    const providers = await db.prepare("SELECT tenant_id,provider,encrypted_config FROM payment_provider_settings WHERE key_version<>? LIMIT 50")
      .bind(keyring.active).all<{ tenant_id: string; provider: string; encrypted_config: string }>();
    for (const row of providers.results ?? []) {
      statements.push(
        db.prepare("UPDATE payment_provider_settings SET encrypted_config=?,key_version=?,updated_at=? WHERE tenant_id=? AND provider=?")
          .bind(await rotateSecret(row.encrypted_config, `payment-provider:${row.provider}`, keyring), keyring.active, now, row.tenant_id, row.provider),
      );
    }
    const totpRows = await db.prepare("SELECT user_id,encrypted_secret FROM totp_credentials WHERE key_version<>? LIMIT 50")
      .bind(keyring.active).all<{ user_id: string; encrypted_secret: string }>();
    for (const row of totpRows.results ?? []) {
      statements.push(
        db.prepare("UPDATE totp_credentials SET encrypted_secret=?,key_version=?,updated_at=? WHERE user_id=?")
          .bind(await rotateSecret(row.encrypted_secret, `totp:${row.user_id}`, keyring), keyring.active, now, row.user_id),
      );
    }
    if (statements.length) await db.batch(statements);
    rotated = statements.length;
  }
  const auditRows = await db.prepare("SELECT id,metadata_json FROM audit_logs ORDER BY created_at DESC LIMIT 500")
  ...
  for (const row of auditRows.results ?? []) {
    ...
    if (sanitized !== row.metadata_json) {
      auditStatements.push(db.prepare("UPDATE audit_logs SET metadata_json=? WHERE id=?").bind(sanitized, row.id));
    }
  }
  if (auditStatements.length) await db.batch(auditStatements);
  auditRowsSanitized = auditStatements.length;
