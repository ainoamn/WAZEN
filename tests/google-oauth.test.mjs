...
test("google oauth start and callback routes exist with PKCE", () => {
  ...
  assert.match(oauth, /oauthCsrfOk/);
  assert.match(oauth, /googleStartPage/);
  assert.match(oauth, /GOOGLE_CLIENT_INVALID/);
  assert.match(oauth, /GOOGLE_REDIRECT_MISMATCH/);
  assert.match(oauth, /cookieNonce && cookieNonce === parsed\.n/);
  assert.match(oauth, /parsed\.bid && browserId && parsed\.bid === browserId/);
  assert.match(start, /createGoogleOAuthRequest/);
  assert.match(start, /googleStartPage/);
  assert.match(start, /oauthStateCookie\(started\.nonce\)/);
  assert.match(callback, /exchangeGoogleCode/);
  assert.match(callback, /oauthCsrfOk/);
  assert.match(callback, /mapGoogleCallbackError/);
  assert.match(callback, /browserIdFromRequest/);
  ...
  assert.match(form, /GOOGLE_CLIENT_INVALID/);
  assert.match(form, /GOOGLE_ACCESS_DENIED/);
});
