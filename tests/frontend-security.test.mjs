...
test("logged-out sign-in does not paint the home load-error screen", () => {
  ...
  assert.match(adminEntry, /returnTo/);
  assert.doesNotMatch(adminEntry, /adminEntryCookie|wazen_admin_entry|end-session|post_logout/);
  assert.match(adminEntry, /NextResponse\.redirect/);
  const proxy = fs.readFileSync(path.join(root, "proxy.ts"), "utf8");
  assert.doesNotMatch(proxy, /ADMIN_ENTRY_COOKIE|wazen_admin_entry|ADMIN_LOCAL_LOGIN_PATH/);
  const apps = fs.readFileSync(path.join(root, "lib/bhd/apps.ts"), "utf8");
...
test("groups view can merge duplicate accounts and import phone or file contacts", () => {
  ...
  assert.match(panel, /unifyMemberPerson/);
